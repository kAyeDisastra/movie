        ...
        <div class="container">
            ...
            <div class="hero-card">
                <div class="hero-left fade-up">
                    ...
                    <div style="margin-top:1.2rem;display:flex;gap:.75rem;flex-wrap:wrap">
                        <a href="<?php echo e(route('login')); ?>" class="btn-cta">Masuk</a>
                        <a href="<?php echo e(route('register')); ?>" class="btn-cta" style="background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.2);">Daftar</a>
                    </div>
                </div>
            ...
            <div class="hero-carousel fade-up" style="display:flex;justify-content:center;">
                <div class="carousel-container" style="width:800px;height:450px;border-radius:20px;overflow:hidden;border:1px solid rgba(255,255,255,0.08);box-shadow:0 30px 80px rgba(2,6,23,0.8);position:relative;">
                    <?php $__currentLoopData = $films; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $film): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="carousel-slide <?php echo e($index === 0 ? 'active' : ''); ?>" style="position:absolute;inset:0;opacity:<?php echo e($index === 0 ? '1' : '0'); ?>;transition:opacity 1.5s ease-in-out;cursor:pointer;" onclick="window.location.href='<?php echo e(route('films.schedules', $film->id)); ?>'">
                        <img src="<?php echo e($film->poster_image ? asset('storage/uploads/' . $film->poster_image) : 'https://picsum.photos/800/450?random=' . $film->id); ?>" alt="<?php echo e($film->title); ?>" style="width:100%;height:100%;object-fit:cover;">
                        <div style="position:absolute;inset:0;background:linear-gradient(45deg,rgba(0,0,0,0.3),transparent 50%,rgba(0,0,0,0.6));">
                            <div style="position:absolute;bottom:0;left:0;right:0;background:linear-gradient(transparent,rgba(0,0,0,0.9));padding:2rem;">
                                <h2 style="color:white;margin:0 0 .5rem 0;font-size:2.5rem;font-weight:800;"><?php echo e($film->title); ?></h2>
                                <div style="display:flex;gap:1rem;align-items:center;margin-bottom:1rem;">
                                    <span style="color:rgba(255,255,255,0.9);font-size:1rem;"><i class="fas fa-clock" style="margin-right:.5rem;"></i><?php echo e($film->duration); ?> menit</span>
                                    <?php if($film->rating): ?>
                                    <span style="color:var(--accent3);font-weight:700;font-size:1rem;">★ <?php echo e($film->rating); ?>/10</span>
                                    <?php endif; ?>
                                </div>
                                <?php if($film->synopsis): ?>
                                <p style="color:rgba(255,255,255,0.8);margin:0 0 1rem 0;font-size:1rem;line-height:1.5;max-width:600px;"><?php echo e(Str::limit($film->synopsis, 150)); ?></p>
                                <?php endif; ?>
                    ...
                    <div style="position:absolute;bottom:20px;left:50%;transform:translateX(-50%);display:flex;gap:10px;">
                        <?php $__currentLoopData = $films; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $film): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="carousel-dot <?php echo e($index === 0 ? 'active' : ''); ?>" data-slide="<?php echo e($index); ?>" style="width:12px;height:12px;border-radius:50%;background:<?php echo e($index === 0 ? 'white' : 'rgba(255,255,255,0.4)'); ?>;cursor:pointer;transition:all 0.3s;border:2px solid rgba(255,255,255,0.2);"></div>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
                </div>
            </div>
    ...
    <main>
        <div class="container">
            ...
            <div class="grid">
                ...
                <article class="movie-card fade-up" tabindex="0" onclick="window.location.href='<?php echo e(route('films.schedules', $item->id)); ?>'" style="cursor: pointer;">
                    <div class="poster-wrap">
                        <img src="<?php echo e($item->poster_image ? asset('storage/uploads/' . $item->poster_image) : 'https://picsum.photos/300/450?random=' . $item->id); ?>" alt="Poster <?php echo e($item->title); ?>">

                        <div class="poster-overlay">
                            <div style="display:flex;justify-content:space-between;align-items:center;">
                                <button class="play-btn" data-trailer="<?php echo e($item->trailer_url); ?>" aria-label="Putar trailer <?php echo e($item->title); ?>" onclick="event.stopPropagation()">
                                ...
                                <a href="<?php echo e(auth()->check() ? route('studios.show', $item->schedules->first()->studio_id ?? 1) : route('login')); ?>" class="order-btn" onclick="event.stopPropagation()">Pesan</a>
                            </div>
                    ...
                    <div class="meta">
                        ...
                        <div style="display:flex;align-items:center;gap:.5rem;margin-bottom:.5rem">
                            <span style="color:var(--accent3);font-weight:700">★ <?php echo e($item->rating); ?>/10</span>
                        </div>
                        <?php endif; ?>

                        <div class="row-meta">
                            <div class="info"><i class="fas fa-clock" style="margin-right:.45rem"></i><?php echo e($item->duration); ?> menit</div>
                            <?php if($item->director): ?>
                            <div class="info"><i class="fas fa-user-tie" style="margin-right:.45rem"></i><?php echo e($item->director); ?></div>
                            <?php endif; ?>
                        </div>

                        <?php if($item->synopsis): ?>
                        <p style="color:rgba(230,238,248,0.7);font-size:.85rem;margin:.5rem 0;line-height:1.4;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden"><?php echo e($item->synopsis); ?></p>
                        <?php endif; ?>
                        ...
                        <div style="display:flex;gap:.4rem;flex-wrap:wrap;margin-bottom:.5rem">
                            <?php $__currentLoopData = $item->schedules->take(3); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $schedule): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <span style="font-size:.75rem;padding:.25rem .5rem;border-radius:6px;background:rgba(255,255,255,0.04);color:rgba(230,238,248,0.8)"><?php echo e($schedule->studio->name ?? 'Studio'); ?></span>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </div>

                        <div class="action-row">
                            <?php $__currentLoopData = $item->schedules->take(4); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $schedule): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <a href="<?php echo e(auth()->check() ? route('studios.show', $schedule->studio_id ?? 1) : route('login')); ?>" class="time-btn" onclick="event.stopPropagation()"><?php echo e(\Carbon\Carbon::parse($schedule->show_time)->format('H:i')); ?></a>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            <?php if($item->schedules->count() > 4): ?>
                                <span style="color:rgba(230,238,248,0.6);font-size:.8rem;padding:.45rem .9rem">+<?php echo e($item->schedules->count() - 4); ?> lainnya</span>
                            <?php endif; ?>
                        </div>
                    </div>
                </article>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\ThinkPad\movie-ticket-laravel\resources\views/dashboard.blade.php ENDPATH**/ ?>